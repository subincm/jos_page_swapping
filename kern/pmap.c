...
boot_alloc(uint32_t n)
{
    ...
    if ((n!= 0) && (KERNBASE + npages*PGSIZE) <= 
           (uint64_t)(nextfree = ROUNDUP((char *)nextfree+n , PGSIZE)))
    {
        panic("System Out of Memory @ %016x trying to allocate till %016x\n", result, nextfree);
    }
...
x64_vm_init(void)
{
    ...
    cprintf(" boot_alloc before %016x\n", boot_alloc(0));
	pages = (struct PageInfo *)boot_alloc(npages*sizeof(struct PageInfo)); 
    
    cprintf(" boot_alloc after pages %016x\n", boot_alloc(0));
	assert(pages);
    ...
    cprintf(" boot_alloc after envs %016x\n", boot_alloc(0));

	assert(envs);

    tx_descs = (struct tx_desc *)boot_alloc(NUM_TX_DESCS*sizeof(struct tx_desc));
    assert(tx_descs);
    memset(tx_descs, 0, NUM_TX_DESCS*sizeof(struct tx_desc));

    rx_descs = (struct rx_desc *)boot_alloc(NUM_RX_DESCS*sizeof(struct rx_desc));
    assert(rx_descs);
    memset(rx_descs, 0, NUM_RX_DESCS*sizeof(struct rx_desc));
	...
	// Your code goes here:
	boot_map_region(boot_pml4e, UPAGES, ROUNDUP(npages*sizeof(struct PageInfo), PGSIZE),
	...
	// LAB 3: Your code here.
	boot_map_region(boot_pml4e, UENVS, ROUNDUP(NENV*sizeof(struct Env), PGSIZE),
	...
	// Permissions: kernel RW, user NONE
	// Your code goes here: 
	boot_map_region(boot_pml4e, KERNBASE, npages*PGSIZE,
                0, PTE_W | PTE_P);
...
mem_init_mp(void)
{
	...
	uint64_t i, kstacktop_end;
	for(i = 0, kstacktop_end = KSTACKTOP - KSTKSIZE; i < NCPU; i++, kstacktop_end -= (KSTKGAP + KSTKSIZE)){
		boot_map_region(boot_pml4e, kstacktop_end, KSTKSIZE, PADDR(&percpu_kstacks[i]), PTE_W | PTE_P);
	}
...
page_init(void)
{
    ...
    for (i = 0; i < npages; i++) {
        ...
        if (
                ...
                (0 == i) ||
                //  2. Then comes the IO hole [IOPHYSMEM, EXTPHYSMEM], which must
                //     never be allocated.
                //  3. Mark the kernel and other datastructures as in use
                (page2pa(&pages[i]) >= IOPHYSMEM && 
                 (uint64_t)page2kva(&pages[i]) < (uint64_t)boot_alloc(0)) ||
                 ((uint64_t)page2pa(&pages[i]) == MPENTRY_PADDR) ||
                 ((uint64_t)page2pa(&pages[i]) >= MMIOBASE && (uint64_t)page2pa(&pages[i]) < MMIOLIM)
           )
        {
            pages[i].pp_ref = 1;
            continue;
...
{
    struct PageInfo *alloc_page = NULL;
   
    if ((alloc_flags&PG_SWAPPABLE) && get_phy_mem_percentage_remaining() < START_SWAP_AT_PERCENTAGE)
    {
	    cprintf("Available RAM below Configured Threshold, SWAP SUB-SYSTEM CALLED\n");
        if (alloc_flags & PG_SWAPPABLE) 
        {
            alloc_page = get_page_from_swap_ss(); //Get swapped/stolen page
            cprintf("SWAP SUB-SYSTEM PAGE = %016x\n", alloc_page); 
            if(NULL == alloc_page)
                return NULL;
        }
        else
    ...
    page_initpp(alloc_page);

    if(alloc_flags & PG_SWAPPABLE)
        alloc_page->flags = PG_SWAPPABLE;
    
    //cprintf("Allocing %016x\n", alloc_page);
...
pml4e_walk(pml4e_t *pml4e, const void *va, int create)
{
    ...
    // If the corresponding PDPE is not present, allocate one 
    if (!(*pml4e & PTE_P))
    {
        if (!create)
            return NULL;
        ...
        newflag = 1;
        page_incref(newpage);
        *pml4e = page2pa(newpage) | PTE_W | PTE_U | PTE_P;
    }

    if (NULL == (pte = pdpe_walk((pdpe_t *)KADDR(PTE_ADDR(*pml4e)), va, create)))
    {
        if(newflag) {
            page_decref(newpage);
            *pml4e = 0;
...
pdpe_walk(pdpe_t *pdpe,const void *va,int create)
{
    ...
    // If the corresponding pgdir is not present, allocate one 
    if (!(*pdpe & PTE_P))
    {
        if (!create)
            return NULL;
        ...
        newflag = 1;
        page_incref(newpage);
        *pdpe = page2pa(newpage) | PTE_W | PTE_U | PTE_P;
    }

    if (NULL == (pte = pgdir_walk((pde_t *)KADDR(PTE_ADDR(*pdpe)), va, create)))
    {
        if(newflag) {
            page_decref(newpage);
            *pdpe = 0;
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
    ...
    // If the corresponding page is not present, allocate one 
    if (!(*pde & PTE_P))
    {
        if (!create)
            return NULL;
        ...
        if(!newpage)
            return NULL;
        page_incref(newpage);
        *pde = page2pa(newpage) | PTE_W | PTE_U |PTE_P;
    }
...
boot_map_region(pml4e_t *pml4e, uintptr_t la, size_t size, physaddr_t pa, int perm)
{
    ...
    for (;size; la += PGSIZE, pa += PGSIZE, size -= PGSIZE)
    {
        if(NULL == (pte = pml4e_walk(pml4e, (void *)la, 1)))
        {
            return;
        }
...
page_insert(pml4e_t *pml4e, struct PageInfo *pp, void *va, int perm)
{
    ...
    if(NULL == (pte = pml4e_walk(pml4e, va, 1)))
    {
        return -E_NO_MEM;
    }

    // Increment the ref count of the page to prevent the page from 
    // being freed from under our feet.
...
//Called only from sys_page_alloc
int page_insert_env(envid_t envid, pml4e_t *pml4e, struct PageInfo *pp, void *va, int perm)
{
    int ret = 0, is_new_page;

    if(!(pp->flags & PG_SWAPPABLE))
      return page_insert(pml4e, pp, va, perm);
    ...
    if ((*pte & PTE_P))
    {
        page_remove_env(envid, pml4e, va);
    }
    
    *pte = page2pa(pp) | perm | PTE_P;

    assert (pp->flags == PG_SWAPPABLE);
    
    //cprintf("\n[%08x] pp->pp_ref = %d, pp = %016x\n",envid, pp->pp_ref, pp); 

    if (is_new_page)
        add_to_lru_list(pp);
    else
        move_to_lru_list_tail(pp);
	
    int pos = ENVX(envid)/8;
    pp->env_bitmap[pos] |= 1<<(ENVX(envid) % 8);
    //cprintf("Inserting %016x to ENV[%08x]\n", pp, ENVX(envid));
    return 0;
}
//
...
page_remove(pml4e_t *pml4e, void *va)
{
    ...
    if ((page = page_lookup(pml4e, va, &pte)))
    {
        page_decref(page);
        *pte = 0;
...
void
page_remove_env(envid_t envid, pml4e_t *pml4e, void *va)
{
    struct PageInfo *page = NULL;
    pte_t *pte = NULL;
    int pos = ENVX(envid)/8;
    ...
    if ((page = page_lookup(pml4e, va, &pte)))
    {
        if (*pte & PTE_SWAP)
        {
            ...
            release_swap_block_if_needed(block, envid);
            *pte = 0;
            page->env_bitmap[pos] &= ~(1<<(ENVX(envid) % 8));
        }
        else
        {
            page->env_bitmap[pos] &= ~(1<<(ENVX(envid) % 8));
...
mmio_map_region(physaddr_t pa, size_t size)
{
	...
	if(base + size < MMIOLIM){
		boot_map_region(boot_pml4e, base, size, pa, PTE_PCD | PTE_PWT | PTE_P | PTE_W);
		base += size;
		return (void*)ret;
	}
	...
	//
	// Your code here:
	panic("mmio_map_region out of limits");
}
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
    ...
    if((uint64_t)curr_va >= ULIM)
    {
        user_mem_check_addr = curr_va;
        return -E_FAULT;
    }

    for(; len && curr_va < ULIM; len-=consumed, curr_va+=consumed)
    {
        page = page_lookup(env->env_pml4e, (void *)curr_va, &pte);

        if (NULL == (page = page_lookup(env->env_pml4e, (void *)curr_va, &pte)))
            break;

        if( (*pte & (perm | PTE_P)) != (perm | PTE_P))
            break;

        consumed = MIN(len, PGSIZE-PGOFF(curr_va));
    }
