...
env_setup_vm(struct Env *e)
{
	...
	p->pp_ref++;
	memcpy(&e->env_pml4e[PML4(UTOP)], &boot_pml4e[PML4(UTOP)], (PML4(~0) - PML4(UTOP))*sizeof(pte_t));
...
region_alloc(struct Env *e, void *va, size_t len)
{
	...
	for(i = 0; i < len; i+= PGSIZE, start+= PGSIZE)
	{
		assert(pp = page_alloc(0));
		assert(-E_NO_MEM != page_insert(e->env_pml4e, pp, (void *)start, PTE_U | PTE_W));
	}
}
...
load_icode(struct Env *e, uint8_t *binary)
{
    ...
    /* Switch Page Tables temporarily for easier memcpy */
    lcr3(e->env_cr3);
	...
	if (NULL == (stackPage = page_alloc(0)))
		panic("load_icode: %e", -E_NO_MEM);

    assert(-E_NO_MEM != page_insert(e->env_pml4e, stackPage, (void *)(USTACKTOP - PGSIZE), PTE_U | PTE_W));
	
	// LAB 3: Your code here.
    e->elf = binary;

    /* Load up the entry point in tf_rip */
...
env_create(uint8_t *binary, enum EnvType type)
{
	...
	else
	{
        ...
        else if(ENV_TYPE_KERNEL_THREAD == type)
        {
			env->env_tf.tf_ds = GD_KD;
			env->env_tf.tf_es = GD_KD;
			env->env_tf.tf_ss = GD_KD;
			env->env_tf.tf_rsp = KSTACKTOP - (KSTKSIZE + KSTKGAP) * cpunum();
			env->env_tf.tf_cs = GD_KT;
			env->env_tf.tf_eflags &= ~FL_IF;
			env->env_tf.tf_rip = (uintptr_t)binary;
            return;
		}
	...
	for(pdpe_index=0;pdpe_index<=3;pdpe_index++){
		...
		for (pdeno = 0; pdeno < pdeno_limit; pdeno++) {
			...
			for (pteno = 0; pteno < PTX(~0); pteno++) {
				if (pt[pteno] & (PTE_P|PTE_SWAP)){
					page_remove_env(e->env_id, e->env_pml4e, PGADDR((uint64_t)0,pdpe_index,pdeno, pteno, 0));
				}
			}
...
env_run(struct Env *e)
{
	...
	//	   3. Set its env_status to ENV_RUNNING,
