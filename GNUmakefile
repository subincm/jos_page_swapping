...
QEMUOPTS = -m 90 -hda $(OBJDIR)/kern/kernel.img -serial mon:stdio -gdb tcp::$(GDBPORT)
QEMUOPTS += $(shell if $(QEMU) -nographic -help | grep -q '^-D '; then echo '-D qemu.log'; fi)
IMAGES = $(OBJDIR)/kern/kernel.img
QEMUOPTS += -smp $(CPUS)
QEMUOPTS += -hdc $(OBJDIR)/fs/fs.img -hdb $(OBJDIR)/fs/swap-fs.img
IMAGES += $(OBJDIR)/fs/fs.img $(OBJDIR)/fs/swap-fs.img
